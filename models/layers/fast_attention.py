...
def relu_kernel_transformation(
    data, is_query, projection_matrix=None, numerical_stabilizer=0.001
):
...
def favor_attention(
    query, key, value, kernel_transformation, causal, projection_matrix=None
):
    ...
    attention_normalizer = tf.expand_dims(
    ...
    def build(self, input_shape):
        ...
        def _glorot_initializer(fan_in, fan_out):
            ...
            return tf.keras.initializers.RandomUniform(
                minval=-limit, maxval=limit, seed=42
            )

        attention_initializer = _glorot_initializer(
            input_shape.as_list()[-1], self.hidden_size
        )
